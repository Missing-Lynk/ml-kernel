...
| AE closed loop | DONE | `userspace/ml-aed/ml-aed.c` meters the `rro_stats` zone grid and drives sensor exposure, sensor gain and the gain-keyed ISP ladder abscissa from one exposure-table index; CFA/CNF/CM/CM2 hardware proof is tracked by the gate-validation row |
...
| Artosyn power-on glue (venc power domain + clocks) in `probe()` | DONE | Recovered and validated: CRG `0x0a106000` (gate/PMU) + isolation `0x0a102000`; the sequence lives in `patches/0260` |
...
| Functional decode - HEVC decodes | DONE | on HW: source-change + GIC-100 IRQ + DEC_PIC complete; the enabling fixes (power sequencing, keep-awake PM, decoder-capability enable) are `patches/0210`..`0260` (`patches/README.md`) |
