...
/*
 * de3d register bank. The ladder owns 50 registers: the control word at
 * +0x00 (two enable bits), seventeen bit-packed scalar registers between
 * 0x2e10 and 0x2eb4 and the 32-register byte-packed curve at 0x2ebc..0x2f38.
 * Registers with bits owned elsewhere carry a mask of the ladder-owned bits
 * and the apply path read-modify-writes. The buffer-address registers stay
 * with ar_isp_de3d_publish. 0x2e98 is the block's input line-time latch,
 * hardware-written and never stored by the vendor packer, so it is not here.
 * Bit 16 of 0x2e90 is set once by the vendor's init path and then preserved
 * by its apply helper; both captures read it set, so the pack carries it as
 * a constant.
 *
 * The vendor's strength stage on 0x2e1c[13:0] and 0x2e20[24:16] (ctx words
 * 772 and 764, the v*s/50 law with a saturating upper branch) is the
 * identity at its default of 50 and nothing in this stack sets a strength,
 * so it is not carried, like rnr's and lnr's.
 *
 ...
 * fields 0x04/0x08 and 0x7c..0x88, the four flag words and the curve tail
 * come verbatim from the upper record, and working-block word 0x90 blends
 * the lower record's 0x8c against the upper record's 0x90 (the packer's one
 * asymmetric input, measured at both capture points). The two knee registers
 * divide 65532 by a blended field difference, truncating toward zero as the
 * vendor's sdiv does.
 */
