...
- `drivers/media/artosyn/` - air-unit camera capture path: the NT99235 sensor subdev, CSI-2 receiver, VIF front end, ISP processing path, and CVISP output stage, all modules (`CONFIG_VIDEO_ARTOSYN_NT99235` / `_CSI2` / `_VIF` / `_ISP` / `_CVISP`). Its Kconfig is *inserted* next to `drivers/media/i2c/Kconfig` rather than appended, so it stays inside the `MEDIA_SUPPORT` block. See `../docs/camera-stack.md`, and `../docs/camera-isp-recovery.md` for the register-level record.
