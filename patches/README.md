...
- `0250-wave5-vpuapi-h.patch` - the two `struct vpu_instance` fields the above needs: `keep_dma_bufs`, and `active_enc_src_idx` (the OUTPUT buffer the encoder firmware currently owns, consumed by `0280`'s result-error path).
