...
- `0500-dw-axi-dmac-h.patch`, `0510-dw-axi-dmac-platform.patch` - serialize the `DMAC_CFG` read-modify-write behind a new `chip->cfg_lock`. That one controller-wide register holds `DMAC_EN` and `INT_EN`, and mainline read-modify-writes it from a channel start and from the interrupt handler with nothing serializing the two, so on SMP the handler's `INT_EN` restore is lost and the controller stops raising interrupts for good: transfers still complete, no IRQ reaches the GIC, and every dmaengine client blocks to its timeout. `0500` adds the lock, `0510` takes it in the four helpers. Not device-specific; `test_tools/dmablit_race_test.c` reproduces the loss on demand.
