...
# tree's module infra if that script is missing.
...
# In-tree =m modules the open features need: buttons (CONFIG_IIO/INPUT_EVDEV/KEYBOARD_ADC
# -> industrialio.ko, evdev.ko, adc-keys.ko) and the status LED (CONFIG_SPI_DW_MMIO/SPIDEV
# -> spi-dw.ko, spi-dw-mmio.ko, spidev.ko). `scripts/build.sh` builds only Image+dtbs, so
# these are built here. This MUST run before the out-of-tree build below so the IIO symbols
# (which artosyn_adc.ko imports) are written into the tree's Module.symvers; otherwise the
# out-of-tree modpost can't resolve devm_iio_device_* and the .ko silently loses them.
echo "=== building in-tree =m modules (IIO/input/spi) ==="
run "in-tree modules" make -C "$KTREE" ARCH=arm64 CROSS_COMPILE="$TC" modules
