...
static inline u32 ar_readl(int off) { return readl(ctl + off); }
static inline void ar_writel(u32 v, int off) { writel(v, ctl + off); }
...
static void set_clock(unsigned int hz)
{
	...
	ar_writel(div, REG_CLKDIV);
	ar_writel(0, REG_CLKSRC);
	update_clock();

	/* enable card clock (no low-power gating during bring-up) */
	ar_writel(CLKENA_CCLK, REG_CLKENA);
	update_clock();
...
static int send_cmd(u8 idx, u32 arg, u32 flags, u32 *resp)
{
	...
	ar_writel(0xffffffff, REG_RINTSTS);

	/* wait for data-not-busy */
	readl_poll_timeout_atomic(ctl + REG_STATUS, v, !(v & STATUS_DATA_BUSY), 10, 100000);

	ar_writel(arg, REG_CMDARG);
	ar_writel(CMD_START | (use_hold ? CMD_USE_HOLD : 0) | flags | idx, REG_CMD);

	/* wait for command-done */
	ret = readl_poll_timeout_atomic(ctl + REG_RINTSTS, v, v & (INT_CMD_DONE | INT_RTO | INT_HLE),
					10, 500000);
	if (ret) {
		pr_info("CMD%u: no cmd-done (RINTSTS=0x%08x)\n", idx, ar_readl(REG_RINTSTS));
		return -ETIMEDOUT;
	}

	v = ar_readl(REG_RINTSTS);
	if (resp) {
		resp[0] = ar_readl(REG_RESP0);
		resp[1] = ar_readl(REG_RESP0 + 4);
		resp[2] = ar_readl(REG_RESP0 + 8);
		resp[3] = ar_readl(REG_RESP0 + 12);
	}
...
static void ctrl_init(void)
{
	...
	ar_writel(CTRL_RESET | CTRL_FIFO_RESET | CTRL_DMA_RESET, REG_CTRL);
	readl_poll_timeout_atomic(ctl + REG_CTRL, v,
				  !(v & (CTRL_RESET | CTRL_FIFO_RESET | CTRL_DMA_RESET)),
				  10, 100000);
	ar_writel(0xffffffff, REG_RINTSTS);	/* clear all interrupts */
	ar_writel(0, REG_INTMASK);		/* polled: mask all */
	ar_writel(0xffffffff, REG_TMOUT);	/* max response/data timeout */
	ar_writel(0x00ffffff, REG_FIFOTH);	/* permissive FIFO thresholds */
	ar_writel(0, REG_CTYPE);		/* 1-bit bus for init */

	if (en_shift >= 0)
		ar_writel(en_shift, 0x108);	/* ENABLE_SHIFT: sample-point shift */

	if (uhs_reg >= 0)
		ar_writel(uhs_reg, REG_UHS);

	ar_writel(1, REG_PWREN);		/* power slot 0 */
	udelay(1000);
}

/* Release every mapping established by init. Safe on a partial set: the pointers are
 * static (NULL until mapped) and cleared here, so it serves both the init error path
 * and module exit. module_exit does not run when init fails, so init must unwind itself.
 */
static void artosyn_mmc_unmap(void)
{
	void __iomem **slots[] = { &ctl, &clk_en, &clk_sel_r, &clk_cfg_r };
...
/* SDIO probe: CMD5 IO_SEND_OP_COND. R4 (48-bit, NO CRC). arg=0 queries the OCR; then
 * resend with the voltage window until the chip reports ready (bit31). If the AR8030
 * answers this at all, the chip is alive (was the whole problem).
 */
static void probe_sdio(void)
{
	u32 resp[4];
	int ret;
...
/* SD card identification: CMD8, then ACMD41 until the card powers up, then CMD2/CMD3. */
...
static int __init artosyn_mmc_init(void)
{
	 ...
	 */
	if (rst_n) {
		u32 r = ar_readl(REG_RST_N);

		pr_info("RST_n(0x78) before=0x%08x -> pulse low/high (release card reset)\n", r);
		ar_writel(r & ~1u, REG_RST_N);	/* assert (active-low) */
		mdelay(10);
		ar_writel(r | 1u, REG_RST_N);	/* deassert -> chip boots; boot_delay_ms lets ROM come up */
		mdelay(2);
	}
