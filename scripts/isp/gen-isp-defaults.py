...
def isp_writes(path: str, stop: int) -> Iterator[tuple[int, int]]:
    ...
    with open(path) as trace:
        for line in trace:
            ...
            # Traces taken with MMIOTRACE_READS also carry r-prefixed load lines. They
            # share the sequence counter, so they must be skipped rather than parsed as
            # writes.
            if fields[0][0] != 'w':
                continue
...
def main() -> None:
    ...
    with open(args.output, 'w') as header:
        ...
        emit(' *\n')
        emit(' * Generated by scripts/isp/gen-isp-defaults.py. Do not edit.\n')
        emit(' *\n')
        ...
        for page, ncopies, verified, regs in pages:
            ...
            kind = 'recovered' if verified else 'unverified'
            plural = 'y' if ncopies == 1 else 'ies'
            emit(f'\t/* page 0x{page:02x}: {len(regs)} registers, {seen} traced, '
                 f'{len(regs) - seen} {kind}, {ncopies} block cop{plural} */\n')
    ...
    print(f'  {n_traced} traced, {n_recovered} recovered, {n_unverified} unverified')
    print(f'  setup table: {len(setup)} ordered writes over '
          f'{len({off for off, _ in setup})} registers')

    if trim:
        print(f'  trim table: {len(trim)} registers corrected to the streaming vendor')

    if disagree:
        print(f'  {len(disagree)} registers where the static default and the vendor '
              'first write disagree (traced value wins)')
