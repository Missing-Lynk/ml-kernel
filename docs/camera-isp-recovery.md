...
Five ISP stages are recomputed by the vendor from the AE gain abscissa: `rnr`, `lnr`, `de3d`, `cfa` and `cnf`. `rnr`, `lnr` and `de3d` share one blob layout (a header of enable, interpolate, band count and abscissa selector; sixteen `[low, high]` float32 band slots; one payload record per band) and one law: inside a band the record applies verbatim, between bands the fields blend linearly with truncation toward zero. `cfa` and `cnf` use their own record layouts on the same abscissa. Two more, `cm` and `cm2`, share the shape but key on the *other* abscissa the trigger payload carries, and are covered under the correction below rather than here. The driver implements the shared band selection and blend in `ar-isp-ladder.h` and each stage in its own header, deriving every register from the tuning file at a caller-supplied Q8 module parameter; `ml-aed` writes those parameters from its exposure-table index and re-arms through debugfs `ladders`. The proofs are the `scripts/isp/check-*-ladder.py` family and `scripts/isp/check-ladder-c.py`: the same integer arithmetic in Python, refusing to pass unless every capture-covered register reproduces bit-exactly from the blob at the measured vendor operating points.

Those checks prove the recovery, not the driver: the kernel runs the C, and a divergence between the C and the Python restatement is invisible to a check that only ever runs the Python. `scripts/isp/ladder-dump.c` closes that. It includes the stage headers unmodified out of the driver directory, so the host compiler builds the shipped source, and `scripts/isp/check-ladder-c.py` runs it beside the Python models over a spread of abscissae covering the cold band, verbatim and blended interiors, and both clamps. The generated C agrees with the Python models for all seven derived stages at every checked point.
...
Registers in these banks that are not ladder outputs are classified, not assumed: lnr `0x3d10` is never written by the vendor; de3d `0x2e98` is the block's hardware line-time latch and is never written by the vendor, in the whole write trace, and no longer by this driver either: it and the two hardware-written cfa registers reached the configuration through the measured correction pass, which is a state diff rather than a write diff and cannot tell a latch from a setting. `ar_isp_hw_owned` in `ar-isp-main.c` is where the driver declines to replay them; the de3d buffer addresses are driver-owned; the per-stage user-strength laws are the identity at the vendor's default of 50 and nothing in this stack sets a strength, so they are not carried. The de3d control and strength words (`0x2e00` enables, `0x2e1c`, `0x2e20`, `0x2e90`, `0x2eb4`) are packer outputs and derive with the rest; running them at the replayed pre-streaming state instead is what destroyed moving regions even with the threshold registers exact.

The census over the whole tuning blob and bank map is closed: the powers-of-two ladder signature exists for exactly `blc`, `rnr`, `lnr` and `de3d` among enabled stages (plus disabled hits owned by `gib` and two unused tables). Checked and found static: `dpc` (init_config block), `rgb2yuv`, `qgg`, `lms`, `digigain1/2`. `rgb2yuv` is now attributed as well as static: it is the BT.601 full-range matrix, one of four the library carries, selected by `isp_rgb2yuv_set_csc` and packed sign-magnitude Q10. Two further enabled stages recompute through their own record blends on the gain abscissa: `cfa` and `cnf`, both recovered and derived from the tuning file. `cm` and `cm2` recover the same way but read the AEC trigger scalar instead, so they are derived and held at the replayed bank rather than driven. `de3d` no longer appears in the unexplained set at all. The shipped tuning gates AWB off, leaving `wb` and the traced `ccm1` matrix static on this unit; `acm` is bypassed. The remaining moving tone rows are `gamma`, `drc` and `ltm`.
...
| `lnr` | `0x89f18`, stride `0x428`, 11 entries | implemented (`ar-isp-lnr.h`); preserves `0x3d10`, which the vendor packer never writes |
...
| `cm` | 5x7 gain/angle table in `ar-isp-cm.h` | implemented, proved by `scripts/isp/check-cm-ladder.py` |
| `cm2` | 5x1 gain/angle/bounds table in `ar-isp-cm2.h` | implemented, proved by `scripts/isp/check-cm2-ladder.py` |
| `acm` | gate at blob `0x0b00b8` | bypassed in the shipped tuning |

`rnr`, `lnr` and `de3d` all blend between two adjacent entries by an AE-supplied weight, exactly as BLC does, and truncate on the way to the registers. The weight's abscissa is a linear gain multiplier with unity at 1.0 (rnr's band edges are powers of two), and it is **not** the sensor's analogue gain code. It is the AE exposure-table value divided by 256, proven by the heap-table rows that produced the two measured operating points.
...
That first capture measured an abscissa near 6.89x at an operating point where the sensor's analogue gain code was `0x3c`; the mismatch to the sensor code was real, but it was comparing two different quantities. The later heap-table recovery closed the source: `ml-aed` reads the vendor exposure-table entry, writes the Q8 value through `rnr_gain`/`lnr_gain`/`de3d_gain` and now also through `cfa_gain`/`cnf_gain`, then re-arms the gain-keyed banks through debugfs `ladders`. It writes neither `cm_trigger` nor `cm2_trigger`: those take the other abscissa, and the loop leaves both at the driver default.
...
**`acm` is bypassed in the shipped tuning.** The recovered gate is bank `0x7600` bit 0 with bypass-high polarity; the flag at blob `0x0b00b8` reads zero, and the streaming vendor reads bit 0 set. Its registers are carried as the vendor's static bypassed state; the moving colour work is the shared `cfa/cnf/cm/cm2` gate-validation boot.
...
| `af_stats` buffer | extent known (`0x1200`), autofocus is off and no address is published |
...
`ltm_stats` reads 99.8% nonzero right to the last byte of its `0x80000`, but **only the first `0x8000` is the histogram**. That region decodes as 64 tiles of 256 `u16`, and every one of the 64 sums to exactly 8040, which is `(1920/8/2) * (1080/8/2)`: an 8x8 tile grid at a subsample of 2 per axis, accumulation count 1. Past `0x8000` the same 512-byte stride sums to millions rather than 8040, so it is neighbouring heap and not statistics. The vendor module allocates `((tilesX << 8) * tilesY + 0x200) * 2` = `0x8400` bytes for this buffer (`isp_sub_ltm.c`, command `0xb0c`), which matches the `0x8000` of histogram plus a margin, not the `0x80000` the descriptor spans. The nonzero measurement was real; the conclusion drawn from it, that the fill extent is half a megabyte, was not.
...
**The units of that scalar are not established.** It spans 0 to 500, and both a light level in lux and a total gain in percent fit the evidence, with opposite physical meanings but the same self-consistency. `is_aec_trigger_compute_user` copies a 52-byte trigger record and places the selected low/high indices at `+0x1c/+0x20` and the blend weight at `+0x24`. The ISP gamma call site feeds it stage record `+0x3a410` as current state and `+0x3ac98` as cached state; the ISP DRC call site feeds it stage record `+0x3a2a4` as current state and `+0x3ab2c` as cached state. Settling the scalar means tracing the writer of those records.
...
**The group header is an illuminant ladder, so the selector's axis is colour temperature.** The `0x38` header at `raw + 0x90d4` is fourteen float32: eight kelvins, four zeros, then the shading strength and the enable, which are the `raw + 0x9104` and `raw + 0x9108` fields `check-lsc-shading.py` already reads as `0.5` and `1`. On nt99235 the kelvins are 2700, 2900, 3700, 4300, 4700, 5300, 6200 and 6800. Groups 1 to 3 carry only the strength and enable and hold zero in the kelvin words, so the ladder belongs to the array rather than to a group, and the array ends after group 3: group 4 and beyond are zero over the whole `0x678`. This ladder is separate from CCM's at `raw + 0x25438` rather than a second reader of it. The two have the same shape and differ in value, 6200 and 6800 here against CCM's 6300 and 6700, and they move independently across the three shipped sensor blobs.

**Two of a group's four grids reach the page, and which two is measured rather than assumed.** A page record holds two grid points as `(a, a, b)` triplets, so a group contributes two distinct values per point. Fitting all four groups against every ordered pair of their grids, over the captured page, gives one answer: group 0, grid 1 in the duplicated field and grid 0 in the third, at 832 of 832 bytes. Grid 2 ties only because grids 1 and 2 are bit-identical in groups 0, 1 and 2; group 3 duplicates grids 2 and 3 instead. Every other combination lands at or below 82.7 percent. `AR_ISP_LSC_BLOB_X` and `AR_ISP_LSC_BLOB_Y` are that pair.

Region A is byte-identical between `out/au-vendor-session/lsc-page-light1.txt` and `lsc-page-bright.txt`, which first differ at `0xc00`, well past the `0x340` fetch. The group the vendor selects held still across those two light levels.
...
**Three registers are answered negatively, which is still an answer.** `af_stats` bank `0x7400` reads 0 after its last write, so autofocus is off and the open driver publishes no AF statistics address, as expected for a fixed-focus FPV lens. Register 8 is not statistics at all: `ltm` and `gtm2` both publish their coefficient page to bank`+0x08`, and `ltm_stats` publishes its output to bank`+0x0c` (`str w2, [x21, #12]` with `x21` the bank base), so `0x2808` is the coefficient input and `0x280c` the statistics output, one of each. `awbs_stats` is named (bank `0x6c00`, gated off, geometry registers `36` by `64` at `+0x50`/`+0x54`) but **its capture shows no statistics structure at any element width tested, so its format is not established**; it feeds AWB, not AE. `rgb_hist_stats` (`0x5c00`) and `rgb_max_stats` (`0x5400`) are never written at all.
...
**The three agree on one AE operating point, and that is what makes them checkable.** cm2's installed field is 30, which needs a gain in `[0.9375, 0.96875)`; no ladder row holds one, so it is an interpolation between rows 1 and 2. The blend fraction is independently pinned by the `lo2` bound at `0x4824`, which `check-cm2-ladder.py` proves interpolates 1022 to 1000 giving 1006, forcing the fraction into `[16/22, 17/22)`. Every fraction in that interval gives `floor(32 * gain)` = 30. The abscissa it implies is about 292, which falls in cm's second band, selecting cm's row 1. `check-cm-ladder.py` reads cm's 5x7 table from blob `0x89d70`, proves the installed `0x483c` field 33 from row-1 gain 1.05, and validates the identity-shaped companion fields at `0x4840..0x484c`. The live CT column is narrowed to columns 0 and 3, which both encode gain 1.05 at that row.
...
**lnr `0x3d14` is owned by the LNR apply path.** Two 9-bit fields are packed at `0x1bbc38` from payload words `0x88` and `0x8c`, each scaled by the strength level at `priv+752` through the formula at `0x1bef08`, which is `trunc(v * s / 50)` clamped at `s = 100`.
...
What stands is a measurement rather than a computation: the dark capture is at this driver's own abscissa, 3938/256, and reads `0x00490049` where the old replay shipped `0x004a004a`. The replay needed a band-3 blend around gain 6.4 to 7.2, so it came from a different operating point. The driver now lets `ar_isp_lnr_apply()` recompute the word from the LNR ladder instead of preserving the trim value.
...
`scripts/isp/check-cm-ladder.py` performs the matching cm-side check: it prints the 5x7 gain/angle table, compares the installed gain field against the encoded records, and keeps the remaining CT-column ambiguity explicit until a later capture or gate validation separates columns 0 and 3. The driver headers `ar-isp-cm.h` and `ar-isp-cm2.h` are also included by `ladder-dump.c`, so `scripts/isp/check-ladder-c.py` keeps these two colour transforms in the same C/Python parity gate as the gain-keyed noise and demosaic stages.

### Gamma/DRC: selector bands are recovered, scalar producer pending

Gamma and DRC pages already generate from the tuning blob: gamma curve records begin at blob `0x26b90`, and DRC dynamic profiles begin at blob `0x17b1c`. The remaining runtime tone gap is the AE selector that chooses which record to build for the current scene. The selector tables are in the blob too: gamma has five bands at `0x26b0c`, DRC has six bands at `0x17a9c`, and both have their interpolation flag set.

**The tone pages are blends, and the DRC page now reproduces byte for byte, 2026-08-14.** The selector returns a low entry, a high entry and a weight, and both modules blend their raw source words the same way: `out = (low * (0x1000 - t) + high * t) >> 12`, with `t` the position in the gap between the two bands as a truncated Q12 fraction (`isp_sub_drc` `0x1a50d8` and `0x1a534c`, `isp_sub_gamma` `0x195a80` and `0x1957d0`). Gamma blends the stored 4096-entry curves and decimates afterwards. A scalar inside a band selects one entry with no blend.

`scripts/isp/check-trigger-scalar.py --pair` closes the loop: a captured DRC page measures the scalar, the scalar goes through the shipped selector, the shipped builders rebuild both pages, and the DRC page comes back byte for byte over its whole 0x2000 including the carried tail banks. It does so for `out/au-tone-tables/pre-drc.bin`, blended 3 to 4 at weight 0.2500 from a scalar of exactly 275, and for `out/au-tuning/live_drc.bin`, a pure profile 4. Gamma keeps a bounded decimation residual of 0 to 5 counts of 4095, mean about 1, identical whether the page is blended or pure, so it is the sampling model and not the blend; `scripts/isp/tone-dump.c` runs the shipped headers host-side for both.

**Correction and new measurement, 2026-08-14.** The trigger payload carries two abscissae and each stage picks one from a flag in its own tuning header. The axes separate them: `rnr`, `lnr`, `de3d`, `cfa` and `cnf` key on a linear gain from 1.0 up in powers of two to 2048, while `gamma`, `drc`, `cm` and `cm2` key on a scalar from 0 to 550. `cm` and `cm2` are therefore tone-selector stages, not gain-keyed ones.

The driver's `cm_trigger`/`cm2_trigger` parameters pin that scalar at the operating point the vendor's traced state measures, 291, rather than leaving the replayed bank. The replayed bank is not a neutral fallback: these stages' runtime values sit past the 1475-entry prefix the setup replay applies, the same cut that left `ccm1` holding an identity, so what survives the replay is early-table state. Measured on the air unit, that state holds `cm` `0x483c` = 41 against the streaming vendor's 33, and `cm2`'s bounds read 16/32/512/768 against 1/2/1006/1023. Deriving at 291 installs the vendor's values on all of them, confirmed by register readback.

One scalar has to serve all four stages, so the pinned tone indices are the entries 291 selects: gamma curve 3 spans [280, 330] and DRC profile 4 spans [290, 380], both verbatim with no blend. Curve 3 against profile 3 was an unreachable pair, since profile 3 ends at 270 and curve 3 does not start until 280.

The selection is also not always a pure pick: it returns a low and a high entry plus a weight, and the module blends. A captured DRC page measures the scalar exactly, because its dynamic banks are the blob's own 20-bit samples with no transform between file and page: `out/au-tone-tables/pre-drc.bin` fits profile 3 into profile 4 at weight exactly 0.2500, a scalar of exactly 275.0, and its gamma page agrees at 0.832 against the 0.8333 that predicts. So the "session A = gamma curve 2, DRC profile 3" reading was a nearest-curve reading of a blend, and every candidate ruled out on that session is still standing.

`scripts/isp/check-trigger-scalar.py` is the measuring instrument. `cm` and `cm2` invert to a scalar interval and `rnr`, in the same sweep, inverts to a gain and hence an AE exposure-table index, so one capture measures both scalars at one moment with nothing paired to a heap dump: index 281 against scalar [282.5, 283.5], index 290 against [290.5, 291.75], index 310 or 311 against [307.5, 360]. The scalar tracks the exposure-table index one for one, offset by 0 to 2 counts, which is the size of the frame skew a sequential mid-stream sweep produces. `exp_index` is the leading candidate and nothing measured contradicts it.

`scripts/isp/check-tone-selector.py` reads those selector bands and proves the two known operating intersections. The capture that matched gamma curve 2 and DRC profile 3 requires a scalar in `[210,250]`; the capture that matched gamma curve 3 and DRC profile 4 requires a scalar in `[290,330]`. When given the two vendor heap dumps, the same checker rules out the obvious measured AE fields as that scalar: `exp_index`, the AE output lux-index/gain/line/target fields, current luma, luma target, settle counter, skip countdown, exposure-table gain in Q8, exposure-table gain as percent, and line count. With `--lib`, it also verifies the CVISP-to-AEC dispatch, frame/output handoff, AE control command `29`, CVISP command `0xb0d` trigger snapshot routing, the gamma/DRC module-local snapshot handlers, the direct scalar helper, the user-record helper layout and the ISP gamma/DRC current/cached trigger records in `libmpp_service.so`: the direct helper maps `s0` plus the blob band table to low/high indices; CVISP forwards `0xb16/0x760` payloads to the AEC module command slot; AEC subcommand `0x760` dispatches to the frame handoff, which maps the 85080-byte input and 124-byte output, calls the process slot, then calls control command `29`; command `29` lands at `0x2642e8` and reads back the internal packed-output fields at `+128` and `+144`; command `0xb0d` snapshots the trigger-state block from the payload source at `+0x688`, copies the source payload into the stage snapshot area and broadcasts it to active output modules; gamma copies `source+0x9b4` to module-private `+0x244` and DRC copies it to module-private `+0x264`, both for `0x8c` bytes. The remaining trace is the downstream routing from the AEC packed result or module-private snapshot into the ISP stage records. The debugfs `tone` hook rebuilds and republishes the gamma/DRC pages from the current module parameters. After that trace, `ml-aed` can write selectors and trigger the hook when the selected records change.
