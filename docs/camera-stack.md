...
- The LTM coefficient page is the one enabled stage still running a stand-in: the driver fills a fixed identity ramp where the vendor recomputes the page every frame by CLAHE. The publish path is at parity ahead of that compute: the page buffer is double-buffered per the vendor's contract (two 0x4000 halves, the descriptor rewritten to the freshly filled half on every publish, so the half being fetched is never the one being written), and `ar_isp_ltm_page_publish()` installs a replacement page under that discipline. A producer delivers pages through the debugfs `ltm_page` node (write a whole 0x4000 page, committed on close; read returns the half the hardware currently fetches), and the raw histogram head of `ltm_stats` is exposed at `ltm_hist` for a host-side or in-kernel CLAHE consumer. Until that consumer exists the default identity page stands, which is scene-safe.
