...
The census over the whole tuning blob and bank map is closed: the powers-of-two ladder signature exists for exactly `blc`, `rnr`, `lnr` and `de3d` among enabled stages (plus disabled hits owned by `gib` and two unused tables). Checked and found static: `dpc` (init_config block), `rgb2yuv`, `qgg`, `lms`, `digigain1/2`. `rgb2yuv` is now attributed as well as static: it is the BT.601 full-range matrix, one of four the library carries, selected by `isp_rgb2yuv_set_csc` and packed sign-magnitude Q10. Two further enabled stages recompute on the AEC trigger through their own record blends rather than the shared ladder layout and still run replayed state here: `cfa` and `cnf`; their transforms are the next recovery. `de3d` no longer appears in the unexplained set at all. The AWB-triggered family (`ccm1`, `cm`, `cm2`, `acm`, `wb`, and LSC's selection among its four illuminant groups) and the tone tables (`gamma`, `drc`, `ltm`) recompute from their own 3A inputs and wait on the AWB and AE loops.
